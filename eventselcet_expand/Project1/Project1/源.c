...
//	SOCKET socketarr[NUM];//socket 数组
//	WSAEVENT eventarr[NUM];//eventarr 事件数组
//
//
//};
...
//		{
//			DWORD nes = WSAWaitForMultipleEvents(1, &arr.eventarr[i], FALSE, 0, FALSE);//向操作系统询问事件是否发生 一个一个问
//
...
//			if (SOCKET_ERROR == WSAEnumNetworkEvents(arr.socketarr[i], arr.eventarr[i], &lpNetevents))
//			{
//
//				int a = WSAGetLastError();
//
...
//				{
//					SOCKET socketclient = accept(arr.socketarr[i], NULL, NULL);//创建客户端
...
//				{
//					if (SOCKET_ERROR == send(arr.socketarr[i], " connect success", strlen("connect success"), 0))
//					{
//						int a = GetLastError();
...
//					char buff[1500] = { 0 };
//
//					if (SOCKET_ERROR == recv(arr.socketarr[i], buff, 1499, 0))
//					{
//						int a = GetLastError();
...
//				}
//				closesocket(arr.socketarr[i]);
//				arr.socketarr[i] = arr.socketarr[arr.count - 1];//删除socket 套接字
//				WSACloseEvent(arr.eventarr[i]);
//				arr.eventarr[i] = arr.eventarr[arr.count - 1];//删除event   事件
//				//数量减一
//				arr.count--;//一定要记得写这个！
